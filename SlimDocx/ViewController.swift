...
class ViewController: NSViewController {
    ...
    private func setupDropView() {
        print("🔗 Setting up drop view...")
        guard let dropView = dropView else {
            print("❌ CRITICAL: dropView outlet is nil!")
            return
        }
        print("✅ dropView outlet connected")
        
        dropView.delegate = self
        dropView.wantsLayer = true
        dropView.layer?.backgroundColor = NSColor.controlBackgroundColor.cgColor
        dropView.layer?.borderColor = NSColor.separatorColor.cgColor
        dropView.layer?.borderWidth = 2
        dropView.layer?.cornerRadius = 8
        print("✅ Drop view configured")
    }
    
    private func setupUI() {
        print("🔗 Setting up UI...")
        guard let statusLabel = statusLabel else {
            print("❌ CRITICAL: statusLabel outlet is nil!")
            return
        }
        guard let progressIndicator = progressIndicator else {
            print("❌ CRITICAL: progressIndicator outlet is nil!")
            return
        }
        print("✅ UI outlets connected")
        
        statusLabel.stringValue = "Drop DOCX files here to compress them"
    ...
    private func processDocxFile(at url: URL) {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            let success = self?.compressDocxFile(at: url) ?? false
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    ...
                    self?.statusLabel.stringValue = "✅ Created \(originalName) - compressed.docx"
                } else {
                    self?.statusLabel.stringValue = "❌ Failed to compress \(url.lastPathComponent)"
                }
                
                DispatchQueue.main.asyncAfter(deadline: .now() + 4) {
                    self?.statusLabel.stringValue = "Drop DOCX files here to compress them"
                }
            }
        }
    }
    
    private func compressDocxFile(at fileURL: URL) -> Bool {
...
extension ViewController: DropViewDelegate {
    func dropView(_ dropView: DropView, didReceiveFiles fileURLs: [URL]) {
        print("🎯 DROP DETECTED! Received \(fileURLs.count) files")
        print("📁 Files: \(fileURLs.map { $0.lastPathComponent })")
        
        let docxFiles = fileURLs.filter { $0.pathExtension.lowercased() == "docx" }
        print("📄 DOCX files found: \(docxFiles.count)")
        
        if docxFiles.isEmpty {
            print("❌ No DOCX files in drop")
            statusLabel.stringValue = "❌ Please drop only DOCX files"
        ...
        for docxFile in docxFiles {
            print("🎯 Processing DOCX: \(docxFile.path)")
...
class DropView: NSView {
    ...
    override func draggingEntered(_ sender: NSDraggingInfo) -> NSDragOperation {
        print("🖱️ DRAG ENTERED!")
        isDragOver = true
        needsDisplay = true
        
        let pasteboard = sender.draggingPasteboard
        guard let fileURLs = pasteboard.readObjects(forClasses: [NSURL.self]) as? [URL] else {
            print("❌ No file URLs found in pasteboard")
            return []
        }
        
        print("📁 Dragged files: \(fileURLs.map { $0.lastPathComponent })")
        let hasDocxFiles = fileURLs.contains { $0.pathExtension.lowercased() == "docx" }
        print("📄 Has DOCX files: \(hasDocxFiles)")
        return hasDocxFiles ? .copy : []
    }
    ...
    override func performDragOperation(_ sender: NSDraggingInfo) -> Bool {
        ...
        let pasteboard = sender.draggingPasteboard
        guard let fileURLs = pasteboard.readObjects(forClasses: [NSURL.self]) as? [URL] else {
            print("❌ No file URLs in perform drag operation")
            return false
        }
        
        print("📁 Performing drop with files: \(fileURLs.map { $0.lastPathComponent })")
        delegate?.dropView(self, didReceiveFiles: fileURLs)
        return true
    }
    ...
    override func draw(_ dirtyRect: NSRect) {
        ...
        let icon = NSImage(systemSymbolName: "doc.zipper", accessibilityDescription: "Document compression")
        let iconSize = NSSize(width: 64, height: 64)
        let iconRect = NSRect(
            x: (bounds.width - iconSize.width) / 2,
            y: (bounds.height - iconSize.height) / 2 + 20,
        ...
        let textRect = NSRect(
            ...
            y: (bounds.height - textSize.height) / 2 - 30,
